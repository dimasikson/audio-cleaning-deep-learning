...
def load_batch(fnames, batch_labels):
  ...
  orig_lens = [len(i) for i in batch_labels]
  ...
  for i, val in enumerate(batch_labels):
    batch_labels[i] = np.pad(val, [0, maxLen-val.shape[0]], mode="constant")

  return np.asarray(out), np.asarray(batch_labels), orig_lens
